...
Use this when you want to connect to a GhostForge server from another device such as a phone, tablet, or separate desktop app.
...
- HTTPS/WSS-compatible endpoints work from `https://app.ghostforge.dev`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.ghostforge.dev/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
with the same server-side behavior as `ghostforge serve --tailscale-serve`, then the server asks Tailscale
...
For `https://app.ghostforge.dev`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `ghostforge serve`.

```bash
npx ghostforge serve --host "$(tailscale ip -4)"
```

`ghostforge serve` starts the server without opening a browser and prints:
...
Use `ghostforge serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx ghostforge serve --tailscale-serve
```
...
```bash
npx ghostforge serve --tailscale-serve --tailscale-serve-port 8443
```

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `ghostforge project ...` on the server machine instead.
> Full GUI support for remote project management is coming soon.

### Option 3: Desktop-Managed SSH Launch

Use this when you want the desktop app to start or reuse GhostForge on another machine over SSH.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.ghostforge/ssh-launch/<host-key>/`, starts or reuses a remote T3 server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. GhostForge uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, GhostForge first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path GhostForge uses:
...
1. `ghostforge serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.ghostforge.dev` can save a remote backend in browser local storage from a URL like:
...
Hosted pairing does not proxy traffic through GhostForge. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `ghostforge auth` to manage access after the initial pairing flow.
...
Use `ghostforge auth --help` and the nested subcommand help pages for the full reference.
...
- Use `ghostforge auth` to revoke credentials or sessions you no longer trust.
