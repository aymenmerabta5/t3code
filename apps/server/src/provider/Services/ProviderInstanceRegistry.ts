/**
 * ProviderInstanceRegistry â€” the single Effect service in the new model.
 *
 ...
 * scope and rebuilds â€” that's the entire hot-reload story.
 *
 * What rest-of-server reads from here:
 *   - `getInstance(instanceId)` â€” for routing turn/session calls.
 *   - `listInstances` â€” for snapshot aggregation in `ProviderRegistry`.
 *   - `listUnavailable` â€” `ServerProvider` shadows for instances whose
 *     driver is not registered in this build (rollback / fork tolerance).
 *   - `streamChanges` â€” coalesced "registry mutated" pings so consumers
 *     can re-pull lists or re-broadcast.
 *
 * @module provider/Services/ProviderInstanceRegistry
 */
import type { ProviderInstanceId, ServerProvider } from "@ghostforge/contracts";
...
export interface ProviderInstanceRegistryShape {
   ...
   * id is unknown â€” callers branch on falsy and emit
   ...
   * change â€” instance added, removed, or rebuilt. The payload is `void`
   ...
   * publish â€” the forked fiber may not have subscribed yet when the
   ...
   * starts executing â€” a consumer that `forkScoped`s the stream
...
>()("ghostforge/provider/Services/ProviderInstanceRegistry") {}
