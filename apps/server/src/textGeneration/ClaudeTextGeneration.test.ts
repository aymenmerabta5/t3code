import { ClaudeSettings, ProviderInstanceId } from "@ghostforge/contracts";
...
const ClaudeTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ghostforge-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "ghostforge-claude-text-" });
