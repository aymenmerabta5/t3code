...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@ghostforge/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ghostforge-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__GHOSTFORGE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__GHOSTFORGE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "ghostforge-codex-text-" });
