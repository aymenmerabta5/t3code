    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          GHOSTFORGE_PORT: "9999",
          GHOSTFORGE_MODE: "desktop",
          GHOSTFORGE_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "ghostforge-desktop-backend-config-test-",
    });
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.GHOSTFORGE_PORT);
        assert.isUndefined(first.env.GHOSTFORGE_MODE);
        assert.isUndefined(first.env.GHOSTFORGE_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.ghostforgeHome, environment.baseDir);
