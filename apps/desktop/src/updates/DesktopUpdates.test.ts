...
import type { DesktopUpdateState } from "@ghostforge/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/ghostforge-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    appVersion: "1.2.3",
    appPath: "/repo",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          GHOSTFORGE_HOME: `/tmp/ghostforge-desktop-updates-test-${process.pid}`,
          GHOSTFORGE_DESKTOP_MOCK_UPDATES: "true",
          GHOSTFORGE_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
