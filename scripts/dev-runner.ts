...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".ghostforge"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@ghostforge/contracts",
    "--filter=@ghostforge/web",
    "--filter=t3",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=t3"],
  "dev:web": ["run", "dev", "--filter=@ghostforge/web"],
  "dev:desktop": [
    "run",
    "dev",
    "--filter=@ghostforge/desktop",
    "--filter=@ghostforge/web",
    "--parallel",
  ],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("GHOSTFORGE_PORT_OFFSET"),
  devInstance: optionalStringConfig("GHOSTFORGE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid GHOSTFORGE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `GHOSTFORGE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric GHOSTFORGE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed GHOSTFORGE_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly ghostforgeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(ghostforgeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      GHOSTFORGE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.GHOSTFORGE_PORT = String(serverPort);
      ...
      delete output.GHOSTFORGE_MODE;
      delete output.GHOSTFORGE_NO_BROWSER;
      delete output.GHOSTFORGE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.GHOSTFORGE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.GHOSTFORGE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.GHOSTFORGE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.GHOSTFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.GHOSTFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.GHOSTFORGE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.GHOSTFORGE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.GHOSTFORGE_MODE = "web";
      delete output.GHOSTFORGE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read GHOSTFORGE_PORT_OFFSET/GHOSTFORGE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      ghostforgeHome: input.ghostforgeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.GHOSTFORGE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.GHOSTFORGE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  ghostforgeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all GhostForge data (equivalent to GHOSTFORGE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("GHOSTFORGE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to GHOSTFORGE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("GHOSTFORGE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to GHOSTFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("GHOSTFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to GHOSTFORGE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("GHOSTFORGE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to GHOSTFORGE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("GHOSTFORGE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to GHOSTFORGE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("GHOSTFORGE_PORT")),
  ),
