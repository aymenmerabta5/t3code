...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `ghostforge` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@ghostforge/shared/git`, `@ghostforge/shared/DrainableWorker`) — no barrel index.
