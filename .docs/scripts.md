...
- Dev commands default `T3CODE_STATE_DIR` to `~/.ghostforge/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.ghostforge-2`
...
- Desktop production windows load the bundled UI from `ghostforge://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `ghostforge` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
