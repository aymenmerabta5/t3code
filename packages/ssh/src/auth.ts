...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@ghostforge/ssh/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "ghostforge-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when GhostForge re-runs ssh with a cached password
...
printf 'GhostForge ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when GhostForge re-runs\r
...
[Console]::Error.WriteLine("GhostForge ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "ghostforge-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY ? {} : { DISPLAY: "ghostforge" }),
  };
});
